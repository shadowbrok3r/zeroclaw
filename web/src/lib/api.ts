...
/**
 * Rename a persisted session: PUT /api/sessions/{id} with `{name}`.
 *
 * Accepts the full DB key — gateway `gw_<uuid>` and channel-composite keys
 * alike — because the handler resolves ids verbatim first, then through the
 * `gw_` / `rpc_` / `cc_` prefixes. Answers 404 when the gateway has no
 * metadata row for the session, notably when session persistence is disabled.
 */
export function renameSession(
  sessionKey: string,
  name: string,
): Promise<{ session_id: string; name: string }> {
  // A listing fetched before this rename settled would report the old name on
  // a post-rename refresh, so drop it whether or not the write succeeded.
...
/** Delete a persisted session by its full DB key (same id resolution as
 * {@link renameSession}). */
