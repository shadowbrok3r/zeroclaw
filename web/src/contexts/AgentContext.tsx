...
export interface SessionSocket {
  ...
  /**
   * One-shot ownership override, sent as `?adopt=true`. Mutable so the
   * accepted `session_start` can clear it before the client's own automatic
   * reconnects re-send it. Optional: only the real WebSocket client carries
   * an ownership identity, so headless test sockets may omit it.
   */
  adopt?: boolean;
  connect(): void;
  disconnect(): void;
...
export interface AgentSessionRuntime {
  createSocket(options: { agentAlias: string; sessionId: string; adopt?: boolean }): SessionSocket;
...
export function AgentProvider({
  ...
  // Socket rebuild key. A thread adoption has to reopen the connection without
  // changing the conversation, which none of the socket effect's other
  // dependencies express; adoptThread bumps this instead of hand-rolling a
  // second teardown path beside the effect.
  ...
  // session_start arrives, so routine reconnects and conversation switches
  // never re-send the ownership override.
  ...
  // selected session's persisted history. `socketGeneration` re-runs it for a
  // thread adoption, which reopens the same conversation under an ownership
  // override rather than moving to another one.
  useEffect(() => {
    const ws = sessionRuntime.createSocket({
      agentAlias,
      sessionId,
      // Normally false; adoptThread arms it for exactly one rebuild and the
      // accepted session_start clears it again.
      adopt: adoptNextConnectRef.current,
    });
  ...
  const resetTranscriptState = useCallback(() => {
    ...
    setTyping(false);
    setPendingApproval(null);
    // Resume indicator and ownership refusal both describe the connection to
    // the conversation being left, so they die with it.
    setSessionStart(null);
    setOwnershipConflict(null);
    // A pending adopt intent belongs to the thread it was requested on; a
    // conversation change must not carry the override to an unrelated session.
    adoptNextConnectRef.current = false;
   ...
   * thread, so staying on it is not a fallback and the gate that normally
   * protects an unsaved transcript has nothing left to protect.
   */
  const startNewThread = useCallback((): boolean => {
    if (ownershipConflict) {
      return transitionToSession(sessionRuntimeRef.current.mintId(), true);
    }
   ...
   * current thread to this agent. The one-shot flag is cleared when the adopted
   * connection's session_start arrives (or on any conversation change), so
   * routine reconnects never re-send the override. Never armed automatically.
   */
