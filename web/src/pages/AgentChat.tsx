...
import { Send, Square, Bot, User, AlertCircle, Copy, Check, X, Trash2, Minimize2, Maximize2, ChevronDown, Wrench, BarChart2, ArrowDown, FolderOpen, ImagePlus, Loader2, History } from 'lucide-react';
...
export function AgentChatInner({
  ...
  const runCommand = useCallback((trimmed: string): boolean => {
    ...
    switch (command) {
      ...
      // leave this one on the gateway (issue #7543). The abandoned thread's
      // server transcript stays browsable in the Threads panel.
      case 'new':
        // startNewThread, not startNewSession: the threads wrapper carries the
        // ownership escape hatch, so /new still works when the gateway refuses
        // to resume the current thread. No success notice — a hydration for the
        // freshly minted id lands right after this handler and would overwrite
        // it, and the cleared transcript already says the thread changed.
        if (!startNewThread()) {
  ...
  const handleSend = () => {
    ...
    // Sending is an explicit "follow the conversation" gesture: re-arm
    // auto-scroll so the reply streams into view even if the user had
    // scrolled back.
    setAutoScroll(true);
    writeInput('');
    clearDraft();
      ...
      <div className="border-t border-pc-border bg-pc-surface p-4">
        ...
        <div className="flex items-end gap-3 max-w-4xl mx-auto">
          ...
          <textarea
            ...
            disabled={!connected || typing || !hydrated || !!pendingApproval}
