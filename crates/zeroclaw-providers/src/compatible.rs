...
fn is_empty_turn(text: Option<&str>, tool_calls: &[ProviderToolCall], truncated: bool) -> bool {
    !truncated && tool_calls.is_empty() && text.is_none_or(|t| t.trim().is_empty())
}
...
impl ModelProvider for OpenAiCompatibleModelProvider {
    ...
    async fn chat_with_tools(
        ...
        loop {
            ...
            let truncated = is_length_truncation(finish_reason.as_deref());
            ...
            if is_empty_turn(result.text.as_deref(), &result.tool_calls, truncated)
