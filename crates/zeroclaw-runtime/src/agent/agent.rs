...
impl Agent {
    ...
    async fn execute_tool_call(&self, call: &ParsedToolCall) -> ToolExecutionResult {
        ...
        let mut approval_requirement = self
            .approval_manager
            .as_deref()
            .map(|mgr| mgr.approval_requirement(&call.name))
            ...
            let (decision, decision_channel) = if mgr.is_non_interactive() {
                ...
                for (ch_name, ch) in &channels {
                    match ch.request_approval("", &ch_request).await {
                        ...
                        Ok(None) => continue,
                        Err(e) => {
                            ::zeroclaw_log::record!(WARN, ::zeroclaw_log::Event::new(module_path!(), ::zeroclaw_log::Action::Note).with_outcome(::zeroclaw_log::EventOutcome::Unknown).with_attrs(::serde_json::json!({"tool": call.name, "channel": ch_name, "error": format!("{}", e)})), "channel approval request failed");
                        }
                    }
                }
                let approval = match channel_decision {
                    ...
                    None => {
                        ::zeroclaw_log::record!(
                            ...
                            .with_attrs(::serde_json::json!({"tool": call.name})),
