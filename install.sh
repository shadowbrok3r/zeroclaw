...
# returns 404 until `web/dist` is populated. When the caller passes
# required=true (feature `embedded-web`), any of these failures is fatal
# instead: the binary would otherwise embed an empty/stale dist.
build_web_dashboard() {
  src_dir="$1"
  required="${2:-false}"
