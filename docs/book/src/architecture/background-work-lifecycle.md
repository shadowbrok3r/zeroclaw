...
A run is recorded as `ok`, `error`, or `degraded` (the run succeeded but a best-effort delivery failed). An agent run the tool loop stopped, because it reached `max_tool_iterations` or the loop detector's circuit breaker fired, is recorded as `error` with output that starts with `agent job stopped: <reason>`, followed by the final summary when one was produced. Such a run is not retried, since it may already have had side effects. `delivery.mode = "announce"` sends every run's output to the delivery channel; `delivery.mode = "on_failure"` sends only runs that did not succeed.
